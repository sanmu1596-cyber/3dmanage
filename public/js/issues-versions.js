...
// 容器结构：<div class="searchable-select" id="${baseId}-wrap">
//   <input class="searchable-select-input" id="${baseId}-input">
//   <div class="searchable-select-dropdown" id="${baseId}-dropdown"></div>
//   <input type="hidden" id="${baseId}">
// </div>
function initSearchableSelect(baseId, items, currentValue) {
    ...
    // 清空旧绑定（重新打开弹窗时重新绑定，避免残留 listener）
    if (wrap.dataset.searchableInit === '1') {
        // 已初始化过，只更新 items 和 当前值
        wrap._items = items.slice();
        wrap._render();
    } else {
        wrap.dataset.searchableInit = '1';
        wrap._items = items.slice();

        let highlightIdx = -1;
        const escape = (s) => String(s == null ? '' : s).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

        // 渲染下拉列表（基于当前 input.value 过滤）
        wrap._render = function() {
            const q = (input.value || '').trim().toLowerCase();
            const allItems = wrap._items || [];
            const filtered = q ? allItems.filter(name => name.toLowerCase().includes(q)) : allItems;
            highlightIdx = -1;

            if (filtered.length === 0) {
                dropdown.innerHTML = `<div class="searchable-select-empty">未找到「${escape(q)}」</div>`;
                return;
            }
            dropdown.innerHTML = filtered.map((name, i) => {
                let display = escape(name);
                if (q) {
                    const lower = name.toLowerCase();
                    const idx = lower.indexOf(q);
                    if (idx >= 0) {
                        display = escape(name.slice(0, idx))
                            + '<mark>' + escape(name.slice(idx, idx + q.length)) + '</mark>'
                            + escape(name.slice(idx + q.length));
                    }
                }
                const sel = name === hidden.value ? ' selected' : '';
                return `<div class="searchable-select-option${sel}" data-value="${escape(name)}" data-idx="${i}">${display}</div>`;
            }).join('');
        };

        // 选中
        wrap._select = function(name) {
            hidden.value = name;
            input.value = name;
        ...
        input.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowDown') {
                ...
                if (highlightIdx >= 0 && opts[highlightIdx]) {
                    wrap._select(opts[highlightIdx].dataset.value);
                } else if (opts.length === 1) {
                    wrap._select(opts[0].dataset.value);
                }
        ...
        dropdown.addEventListener('mousedown', (e) => {
            ...
            if (!opt) return;
            e.preventDefault(); // 防止 input blur 导致 dropdown 收起
            wrap._select(opt.dataset.value);
        });
        ...
        document.addEventListener('mousedown', (e) => {
            if (!wrap.contains(e.target)) {
                wrap.classList.remove('open');
                // 失焦时如果input值不在 items 中，恢复为当前 hidden 值
                const allItems = wrap._items || [];
                if (input.value && !allItems.includes(input.value)) {
